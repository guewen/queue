...
class ChannelTree extends Component {
    static template = "queue.ChannelTree";
    static props = ["*"];
    ...
    // Convert a CSS class (such as bg-success-subtle) to its background color
    // value (such as #d4edda). As we add an element to the DOM, we can get the
    // color value back. Trick from mass_mailing/static/src/js/mass_mailing_snippets.js
    _getCSSColorValue(className) {
    ...
    nodePopup(channel) {
        ...
        const hints =
            ...
            `<div>${_t("Right-click to create a sub-channel")}</div>` +
            `</div>`;
    ...
    renderNetwork() {
        ...
        const activeColor = this._getCSSColorValue("bg-success-subtle");
        const pausedColor = this._getCSSColorValue("bg-danger-subtle");
